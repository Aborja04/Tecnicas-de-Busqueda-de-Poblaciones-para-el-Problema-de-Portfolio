...
ResultMHDouble AGG_BLX_ALPHA::optimize(ProblemDouble &problem, int maxevals) {
    this->evaluaciones = 0;
...
void AGG_BLX_ALPHA::cruceBLX(Problem<double>& problem, const vector<tSolution<double>>& padres, vector<tSolution<double>>& hijos, int tam_sol){
...
void AGG_BLX_ALPHA::mutar(vector<tSolution<double>>& hijos, int tam_sol, double lo, double hi, vector<bool>& indices_mutados){
...
void AGG_BLX_ALPHA::seleccionTorneo(const vector<tSolution<double>>& poblacion, const vector<double>& fitness_pob, vector<tSolution<double>>& padres, vector<double>& fitness_padres){
    for (int i = 0; i < TAM_POBLACION; i++) {
