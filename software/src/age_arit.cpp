...
ResultMHDouble AGE_Arit::optimize(ProblemDouble &problem, int maxevals) {
    this->evaluaciones = 0;
    ...
    while(evaluaciones < maxevals){
        ...
        cruceAritmetico(problem, poblacion[pos_1], poblacion[pos_2], tam_sol, hijo1, hijo2);
...
void AGE_Arit::cruceAritmetico(Problem<double>& problem, const tSolution<double>& padre1, const tSolution<double>& padre2, int tam_sol, tSolution<double>& hijo1, tSolution<double>& hijo2){
        
    // Cruce Aritmético
    for (size_t j = 0; j < tam_sol; j++) {
        double r = Random::get<double>(0.0, 1.0);
        hijo1[j] = r * padre1[j] + (1.0 - r) * padre2[j];
        hijo2[j] = r * padre2[j] + (1.0 - r) * padre1[j];
    }
...
void AGE_Arit::mutar(Problem<double>& problem, tSolution<double>& hijo, int tam_sol, double lo, double hi){
...
int AGE_Arit::seleccionTorneo(const vector<tSolution<double>>& poblacion, const vector<double>& fitness_pob){
...
void AGE_Arit::seleccionaDosPeores(const vector<double>& fitness_pob, int &indice_peor_1, int &indice_peor_2){
...
void AGE_Arit::seleccionaMejor(const vector<double>& fitness_pob, int &indice_mejor){
