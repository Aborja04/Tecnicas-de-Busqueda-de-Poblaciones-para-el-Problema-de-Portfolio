...
ResultMHDouble AM_ALL::optimize(ProblemDouble &problem, int maxevals) {
    this->evaluaciones = 0;
    ...
    while(evaluaciones < maxevals){
        if(contador_generaciones < 10){
            ...
            for(int t= 0; t < TAM_POBLACION && evaluaciones < maxevals; t++){
                bl.aplicarBL(problem, poblacion[t], fitness_poblacion[t], posiciones, tam_pos, evaluaciones);
            }
...
void AM_ALL::cruceBLX(Problem<double>& problem, const vector<tSolution<double>>& padres, vector<tSolution<double>>& hijos, int tam_sol){
...
void AM_ALL::mutar(vector<tSolution<double>>& hijos, int tam_sol, double lo, double hi, vector<bool>& indices_mutados){
...
void AM_ALL::seleccionTorneo(const vector<tSolution<double>>& poblacion, const vector<double>& fitness_pob, vector<tSolution<double>>& padres, vector<double>& fitness_padres){
    for (int i = 0; i < TAM_POBLACION; i++) {
...
void AM_ALL::inicializarPosiciones(int tam_sol){
    posiciones.clear();
...
int AM_ALL::seleccionaMejor(const vector<double>& fitness_pob){
