...
ResultMHDouble AM_BEST_MOD::optimize(ProblemDouble &problem, int maxevals) {
    this->evaluaciones = 0;
    ...
    vector<int> indices_mejores(N_MEJORES);
    ...
    while(evaluaciones < maxevals){
        ...
        if(contador_generaciones < 10){
            ...
            seleccionaNMejores(poblacion, fitness_poblacion, indices_mejores);
            for(int t= 0; t < N_MEJORES && evaluaciones < maxevals; t++){
                bl.aplicarBL(problem, poblacion[indices_mejores[t]], fitness_poblacion[indices_mejores[t]], posiciones, tam_pos, evaluaciones);
            }
...
void AM_BEST_MOD::cruceBLX(Problem<double>& problem, const vector<tSolution<double>>& padres, vector<tSolution<double>>& hijos, int tam_sol){
...
void AM_BEST_MOD::mutar(vector<tSolution<double>>& hijos, int tam_sol, double lo, double hi, vector<bool>& indices_mutados){
...
void AM_BEST_MOD::seleccionTorneo(const vector<tSolution<double>>& poblacion, const vector<double>& fitness_pob, vector<tSolution<double>>& padres, vector<double>& fitness_padres){
    for (int i = 0; i < TAM_POBLACION; i++) {
...
void AM_BEST_MOD::inicializarPosiciones(int tam_sol){
    posiciones.clear();
...
int AM_BEST_MOD::seleccionaMejor(const vector<double>& fitness_pob){
...
void AM_BEST_MOD::seleccionaNMejores(const vector<tSolution<double>>& poblacion, const vector<double>& fitness_pob, vector<int>& indices_mejores){
    vector<int> indices(TAM_POBLACION);
    iota(indices.begin(), indices.end(), 0);
    
    sort(indices.begin(), indices.end(), [&fitness_pob](int a, int b){
        return fitness_pob[a] > fitness_pob[b];
    });

    indices_mejores.resize(N_MEJORES);
    for(int i = 0; i < N_MEJORES; i++){
        indices_mejores[i] = indices[i];  // guardamos la posicion original
    }
}
